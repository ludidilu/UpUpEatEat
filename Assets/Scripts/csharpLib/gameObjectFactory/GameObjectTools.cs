...
namespace gameObjectFactory
{
    public class GameObjectTools
    {
        ...
        public static Vector3[] GetNormals(Vector3[] _vertex, int[] _index)
        {
            ...
            {
                int tmpIndex = _index[i];

                int tmpIndexx;

                if (sameVertexDic.TryGetValue(tmpIndex, out tmpIndexx))
                {
                    faceVertex[tmpIndexx].Add(i / 3);
                ...
                if (faceVertex.TryGetValue(tmpIndex, out tmpList))
                {
                    tmpList.Add(i / 3);
            ...
            for (int i = 0; i < _vertex.Length; i++)
            {
                ...
                if(sameVertexDic.TryGetValue(i, out tmpIndex))
                {
                    normal[i] = normal[tmpIndex];
                }
                else
                {
                    float area = 0;
